...
import { getAllSetlists, deleteSetlistAdmin } from '@/app/actions/admin';
import { useTitle } from '@/lib/TitleContext';
import { List, Trash2, RefreshCw, User, Calendar } from 'lucide-react';
...
export default function AdminSetlistsPage() {
    ...
    const loadData = async () => {
        ...
        setError('');
        try {
            const data = await getAllSetlists();
            setSetlists(data);
        } catch (err: any) {
            if (err.message === 'Acceso denegado: se requieren permisos de administrador') {
                router.push('/dashboard');
            } else {
                setError(err.message || 'Error al cargar setlists');
            }
        } finally {
            setLoading(false);
        }
    ...
    const handleDelete = async (setlistId: number, setlistName: string) => {
        if (confirm(`¿Eliminar el setlist "${setlistName}" permanentemente?`)) {
            try {
                await deleteSetlistAdmin(setlistId);
                await loadData();
            } catch (err: any) {
                alert(err.message);
            }
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {setlists.map((setlist) => (
                                <tr key={setlist.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                    <td className="px-4 py-3">
                                        <Link href={`/setlists/${setlist.id}`} className="font-medium text-blue-600 dark:text-blue-400 hover:underline">
                                            {setlist.name}
                                        </Link>
                                    </td>
                                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{setlist.description || '—'}</td>
                                    <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{setlist.userId || 'Anónimo'}</td>
                                    <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                                        {setlist.createdAt ? new Date(setlist.createdAt).toLocaleDateString('es-ES') : '—'}
                                    </td>
